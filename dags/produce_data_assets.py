...
s3_bucket_name = "dk-bucket-12-2025"
posts_asset = Asset(f"s3://{s3_bucket_name}/raw/Posts.xml")
users_asset = Asset(f"s3://{s3_bucket_name}/raw/Users.xml")


@asset.multi(schedule="@daily", outlets=[posts_asset, users_asset])
def produce_data_assets():
    # Define variables for downloading and extracting the file
